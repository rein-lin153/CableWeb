<template>
  <div class="flex flex-col h-full bg-white rounded-xl shadow-sm border border-gray-100">
    ...
    <div class="p-6 border-b border-gray-100 flex justify-between items-center">
      ...
      <button @click="openCreateModal" class="bg-gray-900 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-orange-600 transition-colors flex items-center">
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="item in newsList" :key="item.id" class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
              <button @click="openEditModal(item)" class="text-indigo-600 hover:text-indigo-900">编辑</button>
            ...
            <form @submit.prevent="handleSubmit" class="space-y-4">
              ...
              <div class="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                <button type="submit" :disabled="submitting" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-gray-900 text-base font-medium text-white hover:bg-orange-600 sm:col-start-2 sm:text-sm">
                  {{ submitting ? '保存中...' : (editingId ? '确认修改' : '确认发布') }}
                </button>
...
const editingId = ref(null); // 记录当前正在编辑的ID，null表示新增模式
...
const openCreateModal = () => {
  editingId.value = null; // 重置为新增模式
  Object.assign(form, { title: '', summary: '', content: '', image_url: '' }); // 清空表单
  showModal.value = true;
};

// 打开编辑弹窗
const openEditModal = (item) => {
  editingId.value = item.id; // 设置为编辑模式
  // 回填数据
  Object.assign(form, { 
    title: item.title, 
...
const handleSubmit = async () => {
  ...
  try {
    if (editingId.value) {
      // 编辑模式：调用 PUT
      await api.put(`/news/${editingId.value}`, form);
      alert('修改成功');
    } else {
      // 新增模式：调用 POST
