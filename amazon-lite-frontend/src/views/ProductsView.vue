    ...
    <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div class="flex-1 max-w-md mx-4">
          <div class="relative">
            <input v-model="searchQuery" type="text" placeholder="输入型号搜索 (如: YJV 4*16)..."
        ...
        <div class="flex gap-2">
          <button
            class="px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium hover:border-orange-500 hover:text-orange-600 transition-colors shadow-sm whitespace-nowrap">
            仅显示现货
          </button>
        </div>
      </div>
      ...
      <div class="flex flex-col lg:flex-row gap-6">
          ...
          <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden sticky top-24">
            ...
            <nav class="p-2 space-y-1 max-h-[70vh] overflow-y-auto custom-scrollbar">
              <button @click="selectCategory(null)"
                ...
                :class="selectedCategoryId === null ? 'font-bold text-orange-600 bg-orange-50' : 'text-gray-600'">
                全部产品
              </button>

              <div v-for="parent in treeCategories" :key="parent.id" class="mt-2">
                <div @click="toggleCategory(parent)"
                  class="flex items-center justify-between px-3 py-2 text-sm font-bold text-gray-900 cursor-pointer hover:text-orange-600">
                  {{ parent.name }}
                  <span class="text-xs text-gray-400 transform transition-transform"
                    :class="parent.isOpen ? 'rotate-180' : ''">▼</span>
                </div>
                <div v-show="parent.isOpen" class="pl-2 space-y-0.5">
                  <button v-for="child in parent.children" :key="child.id" @click="selectCategory(child.id)"
                    ...
                    <span
                      class="text-[10px] text-gray-300 group-hover:text-gray-400 bg-gray-50 px-1.5 rounded-full border border-gray-100">{{
                        getProductCount(child.id) }}</span>
                  </button>
                </div>
              </div>
            </nav>
        ...
        <main class="flex-1">
          ...
          <div v-else class="space-y-8">
            <section v-for="cat in displayCategories" :key="cat.id"
              class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">

              <div
                class="bg-gray-50/80 px-6 py-4 border-b border-gray-200 flex items-center justify-between backdrop-blur-sm">
                <h2 class="text-lg font-bold text-gray-900 flex items-center">
                  <span class="w-1.5 h-4 bg-orange-500 rounded mr-2"></span>
                  {{ cat.name }}
                </h2>
                <span class="text-xs text-gray-500 bg-white px-2 py-1 rounded border border-gray-200">共 {{
                  getProductsByCategory(cat.id).length }} 个型号</span>
              </div>
              ...
              <div class="hidden md:block overflow-x-auto">
                <table class="w-full text-left border-collapse">
                  ...
                  <tbody class="divide-y divide-gray-100">
                    <tr v-for="product in getProductsByCategory(cat.id)" :key="product.id"
                      class="hover:bg-orange-50/40 transition-colors group cursor-pointer"
                      ...
                      <td class="px-6 py-4">
                        <div class="font-bold text-gray-900 group-hover:text-orange-600 transition-colors text-sm">{{
                          product.name }}</div>
                        <div class="text-[10px] text-gray-400 mt-1 uppercase tracking-wide">{{
                          product.category_detail?.name }}</div>
                      </td>
                      <td class="px-6 py-4">
                        <div class="flex gap-1 flex-wrap">
                          <span
                            class="px-2 py-0.5 rounded text-[10px] bg-gray-100 text-gray-600 border border-gray-200">国标纯铜</span>
                          <span
                            class="px-2 py-0.5 rounded text-[10px] bg-gray-100 text-gray-600 border border-gray-200">耐火阻燃</span>
                        </div>
                      </td>
                      <td class="px-6 py-4">
                      ...
                      <td class="px-6 py-4 text-right">
                        <div class="flex flex-col items-end">
                          ...
                          <div
                            class="text-[10px] text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded mt-1 border border-blue-100">
                      ...
                      <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                          ...
                          <button @click.stop="" title="下载技术规格书 PDF"
              ...
              <div class="md:hidden divide-y divide-gray-100">
                <div v-for="product in getProductsByCategory(cat.id)" :key="product.id"
                  @click="openVariantModal(product)" class="p-4 flex gap-4 active:bg-gray-50 transition-colors">
                  <div
                    class="w-20 h-20 bg-gray-100 rounded border border-gray-200 flex-shrink-0 relative overflow-hidden">
                    <img :src="product.image_url" class="w-full h-full object-cover mix-blend-multiply">
                  </div>
                  <div class="flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <div class="flex gap-1 mt-1">
                        <span
                          class="text-[10px] bg-gray-100 px-1.5 rounded text-gray-500 border border-gray-200">国标</span>
                      </div>
                    </div>
                    <div class="flex justify-between items-end">
                      ...
                      <button
                        class="text-xs bg-gray-900 text-white px-3 py-1.5 rounded font-medium shadow-sm">选规格</button>
                    </div>
                  </div>
                </div>
              </div>

              <div v-if="getProductsByCategory(cat.id).length === 0"
                class="p-12 text-center text-gray-400 text-sm bg-gray-50/30">
                该分类下暂无上架产品
              </div>
            </section>
          </div>
...
// 修改 displayCategories 计算属性
const displayCategories = computed(() => {
  // 1. 如果有搜索词，显示所有包含匹配产品的分类
  if (searchQuery.value.trim()) {
    const query = searchQuery.value.toLowerCase();
    // 找出所有匹配的产品
    const matchedProducts = allProducts.value.filter(p =>
      p.name.toLowerCase().includes(query) ||
      (p.category_detail?.name && p.category_detail.name.toLowerCase().includes(query))
    );
    // 找出这些产品所属的分类ID
    const matchedCatIds = new Set(matchedProducts.map(p => p.category_id));
    // 返回这些分类对象
    return categories.value.filter(c => matchedCatIds.has(c.id));
  }
...
const getProductsByCategory = (catId) => {
  let products = allProducts.value.filter(p => p.category_id === catId);
