<template>
  <div class="bg-white">
      ...
      <div class="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-4 pt-10">
        ...
        <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6 animate-fade-up animation-delay-200">
          <button @click="scrollToProducts"
            class="bg-orange-600 text-white px-8 py-4 rounded-full font-bold text-lg hover:bg-orange-500 transition-all hover:scale-105 shadow-lg shadow-orange-600/30 flex items-center justify-center">
            {{ $t('hero.btn_stock') }}
    ...
    <div class="bg-gray-50 border-y border-gray-200 transition-colors duration-300"
      ...
      <div
        class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-col sm:flex-row justify-between items-center text-sm text-gray-600">
        <div class="flex items-center space-x-4 mb-2 sm:mb-0">
          <div class="flex items-center">
            <span class="w-2.5 h-2.5 rounded-full mr-2 animate-pulse"
              :class="currency === 'CNY' ? 'bg-red-600' : 'bg-blue-600'"></span>
            <span class="font-bold text-gray-900 mr-2">{{ currentPriceData.source }}:</span>
          </div>
          <div class="flex items-baseline space-x-2">
            <span class="font-mono text-xl font-extrabold tracking-tight"
              :class="currency === 'CNY' ? 'text-red-700' : 'text-blue-700'">
          ...
          <span class="text-xs px-2 py-0.5 rounded font-medium flex items-center"
            :class="currentPriceData.change > 0 ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'">
        ...
        <div class="flex items-center space-x-6">
          <div class="flex bg-gray-200 rounded-lg p-1">
            <button @click="currency = 'CNY'" class="px-3 py-1 rounded-md text-xs font-bold transition-all duration-200"
              :class="currency === 'CNY' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'">RMB
              ¥</button>
            <button @click="currency = 'USD'" class="px-3 py-1 rounded-md text-xs font-bold transition-all duration-200"
              :class="currency === 'USD' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'">USD
              $</button>
          </div>
          <div class="hidden md:flex flex-col items-end text-[10px] leading-tight text-gray-400">
            <div>更新于: {{ priceDatabase.updated || '获取中...' }}</div>
            <div v-if="priceDatabase.exchange_rate" class="text-gray-500 font-mono mt-0.5">参考汇率: 1 USD ≈ {{
              priceDatabase.exchange_rate }} CNY</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <main id="product-section" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
      <div class="flex flex-row justify-between items-end mb-8">
        ...
        <button @click="$router.push('/products')"
          class="group flex items-center text-sm font-bold text-orange-600 hover:text-orange-700 transition-colors">
          查看全部产品
          <div
            class="ml-2 w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center group-hover:bg-orange-200 transition-colors">
      ...
      <div v-else>
        <swiper :slides-per-view="1" :space-between="24" :loop="products.length > 4"
          ...
          <swiper-slide v-for="product in products.slice(0, 10)" :key="product.id" class="h-auto">
            ...
            <div
              class="group h-full bg-white rounded-xl border border-gray-100 hover:border-orange-300 hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden relative">
              <div class="aspect-square bg-gray-100 relative overflow-hidden cursor-pointer"
                ...
                <div v-if="isLoggedIn" class="absolute top-3 left-3 z-10">
                  <span
                    class="bg-green-600 text-white text-[10px] px-2 py-1 rounded-sm uppercase tracking-wider font-bold shadow-sm">现货</span>
                </div>
                <img :src="product.image_url" :alt="product.name"
                  class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 mix-blend-multiply">
              </div>

              <div class="p-4 flex flex-col flex-grow">
                <div class="mb-1"><span class="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500">{{
                  product.category_detail?.name || '工业线缆' }}</span></div>
                <h3
                  class="text-base font-bold text-gray-900 group-hover:text-orange-600 transition-colors line-clamp-1 mb-1 cursor-pointer"
                  @click="openVariantModal(product)">
                  {{ product.name }}</h3>
                ...
                <div class="mt-auto pt-3 border-t border-gray-50 flex justify-between items-end">
                  <div class="text-orange-600 font-bold font-mono">
                    ...
                    <span class="text-xs text-gray-400 font-normal">/{{ product.unit || product.variants?.[0]?.unit ||
                      '单位' }}</span>
                  </div>

                  <button @click.stop="openVariantModal(product)"
                    class="text-xs bg-gray-900 text-white px-3 py-1.5 rounded hover:bg-orange-600 transition-all shadow-md flex items-center">
                    选规格
                  </button>
                </div>
              </div>
    ...
    <section class="bg-white border-t border-gray-100 py-20">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div v-for="news in newsList.slice(0, 3)" :key="news.id" @click="$router.push(`/news/${news.id}`)"
            class="group cursor-pointer flex flex-col hover:-translate-y-1 transition-transform duration-300">
            <div class="aspect-video bg-gray-100 rounded-xl overflow-hidden mb-4 relative">
              <img :src="news.image_url"
                class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
              <div class="absolute bottom-0 left-0 bg-orange-600 text-white text-xs font-bold px-3 py-1 rounded-tr-lg">
            ...
            <div
              class="text-sm font-bold text-orange-600 group-hover:translate-x-1 transition-transform flex items-center mt-auto">
              阅读全文 &rarr;</div>
          </div>
        </div>
      </div>
      ...
      <div class="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
        <div class="col-span-1 md:col-span-2">
          <div class="flex items-center mb-6">
            <div
              class="w-8 h-8 rounded-full bg-orange-500 flex items-center justify-center mr-3 shadow-lg shadow-orange-500/20">
              <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
            </div>
            <h3 class="text-white text-2xl font-bold tracking-tight">Amazon <span class="text-orange-500">Cable</span>
            </h3>
          </div>
    ...
    <ProductVariantModal :is-open="isModalOpen" :product="currentProduct" @close="isModalOpen = false"
